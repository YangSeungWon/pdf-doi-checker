...
| 항목 | 등급 | 내용 |
|---|---|---|
| `status` | error/warn | **철회(retracted)·삭제된 논문**. 우려 표명·정정은 warn |
...
arXiv 항목은 ID가 원문에 인쇄돼 있으므로, **제목과 저자 중 하나만 맞아도** 후보로
올립니다. 저자는 맞는데 제목이 다르면 개정판에서 제목을 바꾼 경우라
(`arXiv:2306.05524` 처럼) 버리지 않고 `추정`으로 올려 "인용한 제목이 낡았다"는 걸
알려줍니다.

`github.com` · `huggingface.co` 같은 **소프트웨어·모델 저장소** 인용은 DOI가 없는
것이 정상이므로 검색하지 않고, 대신 **아직 존재하는지 확인**합니다. GitHub·Hugging Face
API는 CORS를 열어 두고 없는 것에 404/401을 주기 때문에 브라우저에서 확인할 수 있습니다.
없어졌으면 `없는 링크`로 빨갛게 표시합니다.

일반 웹페이지 인용은 **생사를 판정하지 않고** 웹 아카이브 사본만 찾아 링크합니다.
인용 링크가 썩었을 때 실제로 필요한 것이 사본이기 때문입니다.

생사 판정을 안 하는 이유는 브라우저에서 신뢰할 수 있게 할 수 없어서입니다.
대상 사이트가 CORS 헤더를 주지 않으므로 `mode:'cors'`는 항상 실패하고,
`no-cors`는 200과 404를 똑같이 `opaque`(status 0)로 줍니다. 도메인이 죽으면
예외가 나서 그것만은 구분되는 듯했지만, 실측에서 **살아 있는 사이트도 예외를
냈습니다**(Tumblr 호스팅 도메인, curl로는 HTTP 200). 살아 있는 인용을 죽었다고
하는 것은 아무 말 안 하느니만 못하므로 그 판정은 넣지 않았습니다.
...
DOI가 해석된 항목은 Crossref의 `updated-by`(Retraction Watch 데이터)를 함께
조회해 **철회·삭제·우려 표명·정정**을 보고합니다. doi.org의 CSL-JSON에는 이
정보가 없어 따로 물어야 하지만, `filter=doi:A,doi:B`로 20건씩 묶어 조회하므로
요청은 얼마 안 늘어납니다 (수십 건 기준 3회, 약 2초).

arXiv **철회(withdrawn)** 는 감지하지 못합니다. 철회 표시가 arXiv 초록 페이지에만
있고 DataCite 메타데이터에는 없으며(`state: findable`, 초록도 원본 그대로),
arXiv API는 CORS를 열어두지 않아 브라우저에서 부를 수 없습니다.

## 책은?

책·챕터는 DOI가 없는 경우가 많고, 있어도 **인용한 판과 다른 판**의 DOI인 일이
잦습니다. 예를 들어 Strauss & Corbin 1998년 판 인용에 대해 Crossref는 2008년
3판(`10.4135/9781452230153`)을 돌려줍니다. 제목·연도가 어긋나므로 제안하지 않고
`못 찾음`으로 둡니다 — 틀린 판의 DOI를 넣게 하는 것보다 낫습니다.
