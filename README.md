...
arXiv 프리프린트나 DOI를 안 적은 저널 항목은 참고문헌 원문을 통째로
Crossref `query.bibliographic`에 넣어 검색합니다. Crossref는 아무거나 1등으로
돌려주기 때문에, 결과를 그대로 믿지 않고 **제목 유사도 · 저자 일치율 · 연도**로
다시 채점해서 확신도를 나눕니다.

- `확실` — 제목 0.90 이상 + 저자 절반 이상 일치 + 연도 어긋나지 않음
- `추정` — 그보다 약함. 특히 **연도만 다른 경우**가 여기 걸리는데,
  프리프린트가 나중에 학회에 실린 경우(arXiv → ACL 등)라 오히려 유용합니다
